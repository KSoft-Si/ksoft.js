...
class KsoftAPI extends EventEmitter {
	...
	constructor(token, options = {}) {
		...
		if (typeof token !== 'string')
			throw new TypeError('[KSoft API] Please make sure your token is a string');
		this.webhook = null;
		...
		this._apis = readdirSync(apiStore).map(i => i.replace('.js', ''));
		this.api = fetch(baseURL).header('Authorization', `Bearer ${this.token}`);
	}
}
...
const handler = {
	get: function(obj, prop) {
		if (obj._apis.includes(prop)) {
			return require(join(apiStore), prop)(this);
		} else {
