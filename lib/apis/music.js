module.exports = class MusicAPI {
	...
	async recommendations(provider, tracks) {
		if (!provider || !tracks) throw new SyntaxError('[KSoft API] Please specify the provider and tracks parameters');
		const allowedProviders = ['youtube_ids', 'youtube', 'spotify'];
		if (!allowedProviders.some(item => item === provider)) throw new RangeError('[KSoft API] Please specify the correct provider. Options: youtube_ids, youtube, spotify');
		...
		} else if (!Array.isArray(tracks)) { throw new TypeError('[KSoft API] Tracks must be either of type String or Array<String>'); }

		if (!tracks.length) throw new RangeError('[KSoft API] Please provide a track to search');
		if (tracks.length > 5) throw new RangeError('[KSoft API] Please provide less than or equal to 5 tracks');
