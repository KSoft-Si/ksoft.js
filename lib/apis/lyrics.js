...
module.exports = class LyricAPI {
	...
	async search(q, options) {
		...
		const res = await this.http.path('/').query(params).send();
	...
	async getArtistById(id) {
		...
		const res = await this.http.path(`/lyrics/artist/${id}/`).send();
	...
	async getAlbumById(id) {
		if (!id) throw new Error('[Ksoft API] Please define an id');
		if (this.cache.has(id)) return this.cache.get(id);
		const res = await this.http.path(`/lyrics/album/${id}/`).send();
		this.cache.set(id, res.json());
		return res.json();
	}
	/**
	...
	async getTrackById(id) {
		if (!id) throw new Error('[Ksoft API] Please define an id');
		if (this.cache.has(id)) return this.cache.get(id);
		const res = await this.http.path(`/lyrics/track/${id}/`).send();
		this.cache.set(id, res.json());
		return res.json();
	}
