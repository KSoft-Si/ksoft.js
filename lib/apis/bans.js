module.exports = class BanAPI {
	...
	async add(banData) {
		...
		if (banData._data) {
			...
			if (!jsonBanData.user || !jsonBanData.proof || !jsonBanData.reason)
				throw new SyntaxError('[KSoft API] user, reason, and proof fields are required');
	...
	async check(userID) {
		...
		if (typeof userID !== 'string')
			throw new TypeError('[KSoft API] userID must be a string');
		try {
			const res = await this.api
				.path('check')
				.query('user', userID)
	...
	async getUpdate(epochDate) {
		if (!epochDate)
			throw new SyntaxError('[KSoft API] Please specify an epochDate. Example: 1539915420');
	...
	async bulkCheck(ids, { bannedOnly, moreInfo, advancedBannedOnly }) {
		...
		if (advancedBannedOnly) {
			return this.getAdvancedBans(this);
		} else {
			try {
				const res = await this.api
					.path('bulkcheck')
