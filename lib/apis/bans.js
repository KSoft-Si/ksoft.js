module.exports = class BanAPI {
	 ...
	 */
	async bulkCheck(ids, options = {}) {
		if (!ids || !Array.isArray(ids)) throw new TypeError('[KSoft API] Please specify an array of IDs');
		const { bannedOnly, moreInfo, advancedBannedOnly } = options;
		const params = { users: ids.join(',') };
		if (bannedOnly) params.banned_only = bannedOnly;
		if (moreInfo) params.more_info = moreInfo;
	 ...
	 */
	async guildMembersCheck(guildMemberCollection, options = {}) {
		const { moreInfo, ignoreBots } = options;
		if (ignoreBots) {
