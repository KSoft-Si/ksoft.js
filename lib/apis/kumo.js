module.exports = class KumoAPI {
	...
	async getSimpleWeather(reportType, q, units, lang, icons) {
		...
		if (!q) throw new SyntaxError('[KSoft API] Please define a search query');
		if (!reportType || !reportTypes.some(report => reportType !== report))
			throw new RangeError(`[KSoft API] Please define a valid report type. Types: ${reportTypes.join(', ')}`);
		const params = { q };
	...
	async getAdvancedWeather(latitude, longitude, reportType, units, lang, icons) {
		...
		if (!reportType || !reportTypes.some(report => reportType !== report))
			throw new RangeError(`[KSoft API] Please define a valid report type. Types: ${reportTypes.join(', ')}`);
		if (!latitude)
			throw new SyntaxError('[KSoft API] please provide a latitude for the location');
		if (!longitude)
			throw new SyntaxError('[KSoft API] please provide a longitude for the location');
		const params = {};
		if (units) params.units = units;
		if (lang) params.lang = lang;
		if (icons) params.icons = icons;
