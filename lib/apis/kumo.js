module.exports = class KumoAPI {
	...
	async search(q, options = {}) {
		if (!q) throw new SyntaxError('[KSoft API] Please define a search query');
		const { fast, more, mapZoom, includeMap } = options;
		const params = { q };
	 ...
	 * @param {String} units Unit types you can select from
	...
	async getSimpleWeather(reportType, q, units, lang, icons) {
		...
		if (!q) throw new SyntaxError('[KSoft API] Please define a search query');
		if (!reportType || !reportTypes.some(report => reportType !== report)) { throw new RangeError(`[KSoft API] Please define a valid report type. Types: ${reportTypes.join(', ')}`); }
		const params = { q };
	 ...
	 */
	async getAdvancedWeather(latitude, longitude, reportType, units, lang, icons) {
		const reportTypes = ['currently', 'minutely', 'hourly', 'daily'];
		if (!reportType || !reportTypes.some(report => reportType !== report)) { throw new RangeError(`[KSoft API] Please define a valid report type. Types: ${reportTypes.join(', ')}`); }
		if (!latitude) { throw new SyntaxError('[KSoft API] please provide a latitude for the location'); }
		if (!longitude) { throw new SyntaxError('[KSoft API] please provide a longitude for the location'); }
		const params = {};
		if (units) params.units = units;
		if (lang) params.lang = lang;
		if (icons) params.icons = icons;
	 ...
	 */
	async geoip(ip) {
		if (!ip) throw new SyntaxError('[KSoft API] Please define an ip address');
		try {
			const res = await this.api
				.path('geoip')
				.query({ ip })
	...
	async convertCurrency(from, to, value) {
		if (!from) throw new SyntaxError('[KSoft API] Please define a currency type to convert from');
		if (!to) throw new SyntaxError('[KSoft API] Please define a currency type to convert to');
		if (!value) throw new SyntaxError('[KSoft API] Please define an amount to convert');
		try {
			const res = await this.api
				.path(`currency`)
				.query({ from, to, value })
