module.exports = class ImageAPI {
	...
	async getImageFromId(snowflake) {
		...
		try {
			const res = await this.api
				.path('image')
				.path(encodeURIComponent(snowflake))
				.send();
			return res.json;
		} catch (err) {
			console.error(`[KSoft API] ${err}`);
	 ...
	 */
	async getRandomWikiHow(nsfw = false) {
		try {
			const res = await this.api
				.path('random-wikihow')
				.query({nsfw})
	...
	async getRandomReddit(subReddit, span, removeNSFW) {
		if (!subReddit) throw new SyntaxError('[KSoft API] Please specify a subreddit');
		const params = {};
